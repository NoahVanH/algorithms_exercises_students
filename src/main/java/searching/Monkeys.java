...
public class Monkeys {
    public static HashMap<String, Integer> monkeyMap = new HashMap<>(); //name,result

    public static Monkey getMonkeyByName(String monkeyName, List<Monkey> input) {
        for (Monkey monkey : input) {
            if (monkey.name.equals(monkeyName)) {
                return monkey;
            }
        }
        throw new IllegalArgumentException("Monkey not found: " + monkeyName);
    }

    public static long DoOperation (long left,long right, char operator){
        switch (operator){
            case '+':
                return left + right;
            case '-':
            ...
            case '/':
                return left / right;
            default:
                throw new IllegalArgumentException("mauvais operator");
        }
    }

    public static Monkey findRootMonkey(List<Monkey> input){
        String rootMonkeyName = "root";
        for (Monkey monkey: input) {
            if(monkey.name.equals(rootMonkeyName)){
                return monkey;
            }
    ...
    public static long evaluateMonkey(Monkey monkey,List<Monkey> input){
        if (monkey instanceof YellingMonkey){
            return ((YellingMonkey) monkey).number;
        } else if (monkey instanceof OperationMonkey) {
            long leftValue = evaluateMonkey(getMonkeyByName(((OperationMonkey) monkey).leftMonkey, input), input);
            long rightValue = evaluateMonkey(getMonkeyByName(((OperationMonkey) monkey).rightMonkey, input), input);



            long result = DoOperation(leftValue, rightValue, ((OperationMonkey) monkey).op);

            monkeyMap.put(monkey.name, (int) result);
            return result;
        }
        return 0;
    ...
    public static long evaluateRoot(List<Monkey> input) {
        ...
        Monkey rootMonkey = findRootMonkey(input);

        return evaluateMonkey(rootMonkey,input);
